...
void banID()
{
    printf("\nCreating Rejecting.txt...\n");
    FILE *fp=fopen("Rejecting.txt", "w");
    if(fp==NULL)
    {
        printf("Error: File is not created yet.\n");
        fclose(fp);
        return;
    }
    printf("Just Enter last roll no to Reject file\nPress 0 to EXIT");
    int input;
    while(1)
    {
        printf("\nEnter Number: ");
...
void studentPanel()
{
    ...
    while(1)
	{
        ...
        {
            printf("\nThis User ID is currently not allowed\nContact Admin for the reason as soon as possible (Press Enter to continue)");
            getch();
            continue;
        }
        if(isVoted(userID)!=0)
        {
            printf("\n  Your PRN entered is already voted\n  Contact Admin Panel as soon as possible");
            getch();
            continue;
        }
